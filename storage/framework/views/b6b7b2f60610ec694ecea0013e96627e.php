<?php $attributes ??= new \Illuminate\View\ComponentAttributeBag;

$__newAttributes = [];
$__propNames = \Illuminate\View\ComponentAttributeBag::extractPropNames(([
    'name',
    'label' => null,
    'options' => [],
    'selected' => null,
    'resourceObjects' => null,
    'search' => false,
    'required' => false
]));

foreach ($attributes->all() as $__key => $__value) {
    if (in_array($__key, $__propNames)) {
        $$__key = $$__key ?? $__value;
    } else {
        $__newAttributes[$__key] = $__value;
    }
}

$attributes = new \Illuminate\View\ComponentAttributeBag($__newAttributes);

unset($__propNames);
unset($__newAttributes);

foreach (array_filter(([
    'name',
    'label' => null,
    'options' => [],
    'selected' => null,
    'resourceObjects' => null,
    'search' => false,
    'required' => false
]), 'is_string', ARRAY_FILTER_USE_KEY) as $__key => $__value) {
    $$__key = $$__key ?? $__value;
}

$__defined_vars = get_defined_vars();

foreach ($attributes->all() as $__key => $__value) {
    if (array_key_exists($__key, $__defined_vars)) unset($$__key);
}

unset($__defined_vars, $__key, $__value); ?>

<?php
    $elementId = $attributes->get('id') ?? $name;
?>

<div <?php echo e($attributes->except('id')->merge(['class' => 'mb-3'])); ?>>
    <?php if($label): ?>
        <label for="<?php echo e($elementId); ?>" class="form-label fw-bold text-primary">
            <?php echo e($label); ?>

            <?php if($required): ?>
                <span class="text-danger" aria-hidden="true">*</span>
            <?php endif; ?>
        </label>
    <?php endif; ?>

    <select
        name="<?php echo e($name); ?>"
        id="<?php echo e($elementId); ?>"
        <?php if($required): ?> aria-required="true" <?php endif; ?>
        <?php echo e($attributes->merge([
            'class' => 'form-select custom-input ' .
                       ($search ? 'select-search ' : '') .
                       ($errors->has($name) ? ' is-invalid ' : '')
        ])); ?>

    >
        <option value="" <?php echo e(empty(old($name, $selected)) ? 'selected' : ''); ?>>
            Selecione uma opção...
        </option>

        <?php $__currentLoopData = $options; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $value => $labelOption): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php
                $isDigital = false;
                if ($resourceObjects && $value) {
                    $item = $resourceObjects->firstWhere('id', $value);
                    $isDigital = $item && isset($item->is_digital) && $item->is_digital;
                }
            ?>

            <option
                value="<?php echo e($value); ?>"
                data-digital="<?php echo e($isDigital ? '1' : '0'); ?>"
                <?php echo e((string) old($name, $selected) === (string) $value ? 'selected' : ''); ?>

            >
                <?php echo e($labelOption); ?>

            </option>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </select>

    <?php $__errorArgs = [$name];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>
    <div class="invalid-feedback">
        <?php echo e($message); ?>

    </div>
    <?php unset($message);
if (isset($__messageOriginal)) { $message = $__messageOriginal; }
endif;
unset($__errorArgs, $__bag); ?>
</div>
<?php /**PATH /var/www/resources/views/components/forms/select.blade.php ENDPATH**/ ?>